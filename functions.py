...
def signal_filter(raw_data, fs, wn):
    filtered_data = signal.filtfilt(
        *signal.butter(8, wn, btype='highpass',
                       analog=False, output='ba',
                       fs=fs), raw_data) if wn is not None else raw_data
...
class AdaptiveVMD:
    ...
    def _vmd(self, K, alpha):

        init = self._spectrum_peak_init(K)
        u, _, _ = VMD(self.signal, alpha, self.tau, K, self.DC, init, self.tol)

        return u

    def _spectrum_peak_init(self, K):
        
        min_prominence=0.2
        fft_amp = np.abs(np.fft.rfft(self.signal))
        freqs = np.fft.rfftfreq(len(self.signal),  d=1/self.fs)
        peaks, properties = find_peaks(fft_amp, prominence=min_prominence)
        prominences = properties['prominences']
        sorted_indices = np.argsort(prominences)[::-1]
        selected_peaks = freqs[peaks[sorted_indices[:K]]]
        initial_f = sorted(selected_peaks)
        initial_w = [f / (self.fs/2) / (2*math.pi) for f in initial_f]
        return initial_w
    
    def _objfunc(self, params):
        K, alpha = params[0], params[1]
        K = int(round(K))
        r = 0
        u = self._vmd(K, alpha)
        rmse = root_mean_squared_error(self.signal, np.sum(u, axis=0))
        for i in range(u.shape[0]-1):
            r_value, p_value = stats.pearsonr(u[i],  u[i+1])
            r = r + np.abs(r_value)
        index = rmse * r
    ...
    def _optimize_params(self):
        ...
        min_val = float('inf')
        best_x = None
        param1_range = range(4, 10)
        param2_range = range(500, 3000, 500)
        total_iterations = len(param1_range) * len(param2_range)
        with tqdm(total=total_iterations, desc="参数优化进度") as pbar:
            for x in product(param1_range, param2_range):
                current_val = self._objfunc(x)
                if current_val < min_val:
                    min_val, best_x = current_val, x
                pbar.update(1)

        K_best, alpha_best = best_x[0], best_x[1]
        
        return K_best, alpha_best

    def decompose(self):

        K, alpha = self._optimize_params()
        modes = self._vmd(K, alpha)
